...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (productId, quantity = 1, product = null) => {
    try {
      ...
      const existingItem = updatedCart.items?.find(item => item.product?._id === productId || item.productId === productId);
      
      if (existingItem) {
        existingItem.quantity += quantity;
      } else {
        updatedCart.items.push({
          product: product || { _id: productId, name: 'Product', price: 0 },
          productId: productId,
          quantity,
  ...
  const updateCartItem = async (productId, quantity) => {
    try {
      ...
      // Fallback to localStorage
      const updatedCart = { ...cart };
      const item = updatedCart.items?.find(item => item.product?._id === productId || item.productId === productId);
